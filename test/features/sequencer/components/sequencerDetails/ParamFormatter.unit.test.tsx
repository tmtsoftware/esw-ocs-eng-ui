...
import { Angle } from '@tmtsoftware/esw-ts/dist/src/models/params/Angle'
...
describe('Parameter Formatter', () => {
  ...
  //expectations
  const formatedEqCoord = `Base: RA=0.0000034280555555555554 DEC=0.0000034280555555555554\nICRS, Catalog="cat123"\nProper Motion=123 123414`
  const formatedCometCoord = `Base: Epoch of Perihelion=11\ninclination=0.0000034280555555555554 degrees\nLong Ascending Node=0.0000034280555555555554 degrees\nArgument of Perihelion=0.0000034280555555555554 degrees\nPerihelion Distance=200 AU\nEccentricity=0.4\n`
  const formatedMinorCoord = `Base: Epoch=2\ninclination=0.0000034280555555555554 degrees\nLong Ascending Node=0.0000034280555555555554 degrees\nArgument of Perihelion=0.0000034280555555555554 degrees\nMean Distance=2 AU\nEccentricity=2\nMean Anomaly=0.0000034280555555555554 degrees\n`
  const formatedSolarSystemCoord = `Base: Jupiter`
  const formatedAltAzCoord = `Base: Alt=0.0000034280555555555554 Az=0.0000034280555555555554`
  const formattedUtcTime = '1970-01-01T00:00:00Z'
  const formattedTaiTime = '2017-09-04T19:00:00.123456789Z'
  const unFormattedIntValue = '123, 12432'

  const testCases: [Parameter<Key>, SequenceCommand, string][] = [
    [eqCoord, setupWithEqCoord, formatedEqCoord],
    [cometCoord, setupWithCometCoord, formatedCometCoord],
    [minorCoord, setupWithMinorPlanetCoord, formatedMinorCoord],
    [solarSystemCoord, setupWithSolarSystemCoord, formatedSolarSystemCoord],
    [altAzCoord, setupWithAltAzCoord, formatedAltAzCoord],
    [utcTime, setupWithUtcTimeKey, formattedUtcTime],
    [taiTime, setupWithTaiTimeKey, formattedTaiTime],
    [intParam, setupWithIntKey, unFormattedIntValue]
  ]
